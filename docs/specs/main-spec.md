...
> Updated 2026-05-06 via feature: PB-026
> Updated 2026-05-07 via feature: PB-025
> Updated 2026-05-07 via feature: PB-026 (FR-37 revised: source domain display is flavor-determined)

- **FR-4**: The system must convert Markdown input to a valid EPUB document
- **FR-5**: The EPUB output must be a valid EPUB 3.0 package containing title and author metadata, a cover image, a cover chapter, and one or more content chapters (see FR-41)
- **FR-36**: Every EPUB produced by the system must include a cover image (JPEG) embedded in the EPUB manifest for display as a library thumbnail in Kindle. The image dimensions are selected via `PAPERBOY_COVER_RESOLUTION` (see FR-39): one of `1264 × 1680` (default), `1072 × 1448`, or `600 × 800`. The image is generated automatically from the document title and author by rendering the active flavor's thumbnail template (see FR-38) through Satori (HTML/CSS → SVG) and rasterizing the SVG to JPEG via `sharp`. The active flavor is configurable; the resolution is configurable; no user-supplied image is required.
- **FR-37**: Every EPUB produced by the system must include a styled cover chapter as the first page of the document. The chapter HTML and CSS are produced entirely by the active flavor's chapter template (see FR-38). The `classic` flavor renders the document title and author only. A `sourceDomain` field (derived from the frontmatter `url` hostname) is passed to every flavor's chapter template via `ChapterInput.sourceDomain` — flavors may display it or ignore it; the `classic` flavor does not display it. Every flavor receives a `ThumbnailInput` object with optional `sourceDomain?: string` for accent color or other keying; flavors may use this field to customize their appearance or ignore it. Every flavor definition may provide an optional `titleWrap?: { readonly maxChars: number; readonly maxLines: number }` to control how the document title is line-wrapped in the thumbnail; the default wrapping is `{ maxChars: 13, maxLines: 4 }`.
- **FR-38**: The system must support a configurable **cover flavor** — a registered visual identity that provides three coordinated templates: a thumbnail builder, a chapter HTML builder, and a chapter CSS string. The active flavor for the install is selected via the `PAPERBOY_COVER_FLAVOR` environment variable (default `classic`). Flavor names that are not registered must cause `loadConfig()` to fail at startup with a `ConfigError` whose message lists the valid flavor names. The system must ship with at least two bundled flavors: `classic` (default, unchanged visual identity) and `brutalist` (newspaper-style masthead with derived accent color and Inter Bold typography). Adding a new flavor must require only a single source-file addition plus one entry in the static flavor registry — no changes to the rendering pipeline, the converter, or the composition roots.
  > Updated 2026-05-11 via feature: PB-027 brutalist cover flavor
- **FR-39**: The system must support a configurable **cover resolution** for the thumbnail JPEG, selected from a fixed set of three predefined Kindle screen sizes: `1264x1680` (default), `1072x1448`, and `600x800`. Selection is via the `PAPERBOY_COVER_RESOLUTION` environment variable. Values outside this set must cause `loadConfig()` to fail at startup with a `ConfigError` whose message lists the valid resolution names. Arbitrary widths and heights are not supported.
...
- **FR-41**: A document is classified as **multi-section** when its rendered HTML contains two or more headings at the minimum heading level present (e.g., if any `<h1>` exists, only `<h1>` elements are counted; otherwise `<h2>`, and so on). A document with zero or one top-level heading is **single-section**. Single-section documents are produced as a single EPUB content chapter. Multi-section documents are split into one EPUB content chapter per top-level heading, enabling Kindle's "Go To" navigation to list each section independently.
- **FR-42**: For multi-section documents, each EPUB chapter must carry the heading's text as its title, and its content must span from that heading element (exclusive — the heading tag is stripped to avoid duplication, since epub-gen-memory prepends the chapter title) to the start of the next top-level heading. The last chapter captures all remaining content.
- **FR-43**: The auto-generated EPUB Table of Contents page (`toc.xhtml`) must be included in the EPUB reading spine only for multi-section documents, so that Kindle's navigation lists it as an entry. For single-section documents, `toc.xhtml` must be excluded from the reading spine (the file remains in the EPUB manifest, as required by the EPUB 3 spec, but is not part of the reading order and does not appear in Kindle navigation).
