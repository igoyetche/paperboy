...
- [x] (2026-05-07) **Step 1.1: Write failing tests** — assert `listCoverResolutionNames()` returns `["1264x1680", "1072x1448", "600x800"]`, `getCoverResolution("1264x1680")` returns `{ name, width: 1264, height: 1680 }`, `isCoverResolutionName("1280x800")` is `false`, `isCoverResolutionName("1264x1680")` is `true`.
- [x] (2026-05-07) **Step 1.2: Implement** `cover-resolution.ts` per the design (`COVER_RESOLUTIONS` `as const`, `CoverResolutionName = keyof typeof COVER_RESOLUTIONS`, four exported helpers).
- [x] (2026-05-07) **Step 1.3: Run** `npm test test/domain/values/cover-resolution.test.ts`; expect green.
- [x] (2026-05-07) **Step 1.4: Commit** `feat: PB-026 add CoverResolution value type`.
...
- [x] (2026-05-07) **Step 2.1: Add interfaces** — `CoverFlavor`, `ThumbnailInput` (incl. `width: number; height: number`), `ChapterInput`, `SatoriNode`, `SatoriStyle` (Paperboy-owned, listing the CSS subset Satori supports — flexbox, padding/margin, borders, color, fontFamily, fontSize, fontWeight, lineHeight, letterSpacing, textTransform, textAlign, opacity, transform, background gradients, borderRadius). No `any`, no `as`.
- [x] (2026-05-07) **Step 2.2: Run** `npm run build`; expect clean compilation.
- [x] (2026-05-07) **Step 2.3: Commit** `feat: PB-026 add CoverFlavor contract to domain ports`.
...
- [x] (2026-05-07) **Step 3.1: Write failing registry test** — `listFlavorNames()` returns at least `["classic"]`; `getFlavor("classic")` returns a value with `buildThumbnail`, `buildHtmlChapter`, `buildCoverCss`; `isFlavorName("nope")` is `false`.
- [x] (2026-05-07) **Step 3.2: Implement `_shared/`:**
...
- [x] (2026-05-07) **Step 3.3: Implement `flavors/index.ts`** — empty registry first: `const FLAVORS = {} as const satisfies Record<string, CoverFlavor>`. The `classic` flavor lands in Phase 2; the registry test will fail until then. Note: leave the registry test in `it.skip` until Task 6 lands `classic`.
- [x] (2026-05-07) **Step 3.4: Run** `npm run build`; expect clean compilation.
- [x] (2026-05-07) **Step 3.5: Commit** `feat: PB-026 add flavors registry skeleton + _shared helpers`.
...
- [x] (2026-05-07) **Step 4.1:** `npm install satori --save`. Verify the version landed in `package.json` and `package-lock.json`.
- [x] (2026-05-07) **Step 4.2:** `npm audit` — clean.
- [x] (2026-05-07) **Step 4.3:** Commit `chore: PB-026 add satori dependency`.
...
- [x] (2026-05-07) **Step 5.1:** Download Source Serif 4 regular 400 TTF (Adobe Fonts repo on GitHub, SIL OFL). Place at `src/infrastructure/converter/assets/fonts/source-serif-regular.ttf`. Verify license file is committed if required by OFL terms (typically `OFL.txt` next to the font).
- [x] (2026-05-07) **Step 5.2:** Write `scripts/verify-assets.mjs` — checks that `dist/infrastructure/converter/assets/cover-icon.png` and `dist/infrastructure/converter/assets/fonts/source-serif-regular.ttf` both exist and are non-empty; exits 1 with a helpful message if not.
- [x] (2026-05-07) **Step 5.3:** Update `package.json` `scripts.postbuild`: chain the existing asset-copy with `node scripts/verify-assets.mjs`.
- [x] (2026-05-07) **Step 5.4:** `npm run build`; expect both assets present in `dist/`.
- [x] (2026-05-07) **Step 5.5:** Commit `feat: PB-026 bundle Source Serif font + add verify-assets postbuild guard`.
...
- [x] (2026-05-07) **Step 6.1: Port `css.ts`** — copy the body of today's `buildCoverCss()` from `cover-templates.ts`. Replace inline color hex codes with imports from `_shared/tokens.ts`.
- [x] (2026-05-07) **Step 6.2: Port `chapter.ts`** — copy the body of today's `buildHtmlChapter()` from `cover-templates.ts`. Signature changes from `(title, author, domain)` to `(input: ChapterInput)` per the new contract; inside, destructure to `title`, `author`, `sourceDomain`.
- [x] (2026-05-07) **Step 6.3: Implement `thumbnail.ts`** — replace today's `buildCoverSvg` with a `buildThumbnail(input: ThumbnailInput): SatoriNode` function. Use flexbox layout (`display: "flex"`, `flexDirection: "column"`, `alignItems: "center"`, `justifyContent: "center"`), proportional sizing via `scale(input.width, 600, baseSize)` for typography. Render kicker ("PAPERBOY"), title lines (one `<div>` per `titleLines[i]`), rule (a `<div>` with `width`, `height`, `background`), author, and the icon (`<img src={input.iconDataUri}>`).
- [x] (2026-05-07) **Step 6.4: Implement `flavors/classic/index.ts`** — exports the `CoverFlavor` object: `{ name: "classic", buildThumbnail, buildHtmlChapter, buildCoverCss }`.
- [x] (2026-05-07) **Step 6.5: Register in `flavors/index.ts`** — replace the empty registry with `{ classic } as const satisfies Record<string, CoverFlavor>`. Un-skip Task 3's registry test.
- [x] (2026-05-07) **Step 6.6: Run** `npm test test/infrastructure/converter/flavors/registry.test.ts`; expect green.
- [x] (2026-05-07) **Step 6.7: Run** `npm run build`; expect clean.
- [x] (2026-05-07) **Step 6.8: Commit** `feat: PB-026 add classic flavor with thumbnail/chapter/css templates`.
...
- [x] (2026-05-07) **Step 7.1: Update tests first** — modify each existing test to pass `getFlavor("classic")` and `getCoverResolution("1264x1680")` as arguments to the corresponding methods. The byte-comparison fixture test stays for now but its fixture path moves to `flavors/classic/fixtures/sample-thumbnail.svg`. Add the new tests:
...
- [x] (2026-05-07) **Step 7.2: Update `CoverGenerator`:**
...
- [x] (2026-05-07) **Step 7.3: Run** `npm test test/infrastructure/converter/cover-generator.test.ts`; iterate until green. The byte-comparison fixture test will fail on the first run because Satori output differs from today's hand-authored SVG — refresh the fixture in Task 9 (deliberately deferred so refresh is the last step).
- [x] (2026-05-07) **Step 7.4: Commit** `feat: PB-026 switch CoverGenerator to flavor + Satori rendering`.
...
- [x] (2026-05-07) **Step 8.3: Update composition roots** — in each of `index.ts`, `cli-entry.ts`, `watch-entry.ts`, after `loadConfig()`:
  ```ts
...
- [x] (2026-05-07) **Step 8.4: Run** `npm run build` and `npm test`; iterate until green (excluding the fixture comparison test).
- [x] (2026-05-07) **Step 8.5: Commit** `feat: PB-026 thread flavor + resolution through converter and composition roots`.
...
- [x] (2026-05-07) **Step 9.1:** Update the fixture comparison test to iterate `listFlavorNames()` and read fixtures from `src/infrastructure/converter/flavors/<name>/fixtures/`. Use `UPDATE_COVER_FIXTURE=1` to write missing or stale fixtures.
- [x] (2026-05-07) **Step 9.2:** Run `UPDATE_COVER_FIXTURE=1 npx vitest run test/infrastructure/converter/cover-generator.test.ts` to bootstrap `flavors/classic/fixtures/sample-thumbnail.svg` and `sample-cover.html`.
- [x] (2026-05-07) **Step 9.3:** Open `flavors/classic/fixtures/sample-cover.html` in a browser; sanity-check it visually matches today's title page.
- [x] (2026-05-07) **Step 9.4:** Open `flavors/classic/fixtures/sample-thumbnail.svg` in a browser; sanity-check the thumbnail looks right at the new 1264 × 1680 default.
- [x] (2026-05-07) **Step 9.5:** Delete `cover-templates.ts` and `test/fixtures/covers/`. Confirm no remaining imports of the deleted file via `grep` / IDE search.
- [x] (2026-05-07) **Step 9.6:** Run `npm run build` and `npm test`; full suite passes.
- [x] (2026-05-07) **Step 9.7:** Commit `feat: PB-026 refresh per-flavor fixtures, delete cover-templates.ts`.
...
- [x] (2026-05-07) **Step 10.1:** Add an `import/no-restricted-paths` rule (or equivalent — check existing config) so files under `src/infrastructure/converter/flavors/<name>/` may import only from:
  - `src/domain/ports`
...
- [x] (2026-05-07) **Step 10.2:** Run `npm run lint` (or equivalent); confirm the rule activates without breaking `classic`.
- [x] (2026-05-07) **Step 10.3:** Add a deliberately-failing experiment locally to verify the rule fires (e.g., `import sharp from "sharp"` in `classic/thumbnail.ts`); revert before commit.
- [x] (2026-05-07) **Step 10.4:** Commit `chore: PB-026 add lint boundary preventing flavors from coupling to renderer`.
...
- [x] (2026-05-07) **Step 11.1: Write failing tests** — six cases total in `config.test.ts`:
...
- [x] (2026-05-07) **Step 11.2: Update `loadConfig()`** — read both env vars, validate via `isFlavorName` / `isCoverResolutionName`, return `ConfigError` with helpful messages. Add `defaultCoverFlavor: FlavorName` and `coverResolution: CoverResolution` to the `Config` type.
- [x] (2026-05-07) **Step 11.3: Replace the hard-coded composition-root values from Task 8** with `config.defaultCoverFlavor` and `config.coverResolution` lookups.
- [x] (2026-05-07) **Step 11.4:** Run full test suite — green.
- [x] (2026-05-07) **Step 11.5:** Commit `feat: PB-026 validate cover flavor and resolution env vars at startup`.
...
- [x] (2026-05-07) **Step 12.1:** Add a "Cover (PB-026)" section documenting both env vars with their defaults, valid values, and a brief explanation of what each controls.
- [x] (2026-05-07) **Step 12.2:** Commit `docs: PB-026 document PAPERBOY_COVER_FLAVOR and PAPERBOY_COVER_RESOLUTION in .env.example`.
...
- [x] (2026-05-07) **Step 13.1:** `npm run build` — clean.
- [x] (2026-05-07) **Step 13.2:** `npm test` — all tests pass; expect ≥340 passing (337 prior + new).
- [x] (2026-05-07) **Step 13.3:** `npm run sonar:local` — review issues; resolve any new bugs/vulnerabilities introduced; confirm hotspots are safe.
- [x] (2026-05-07) **Step 13.4: End-to-end smoke test** — generate an EPUB through the CLI (`paperboy --title "Smoke Test" --file scripts/sample.md`), open it on a real Kindle device, verify:
...
- [x] (2026-05-07) **Step 13.5: Resolution smoke test** — repeat 13.4 with `PAPERBOY_COVER_RESOLUTION=600x800` and `=1072x1448`; confirm each renders without layout collapse.
- [x] (2026-05-07) **Step 13.6: Flavor failure smoke test** — set `PAPERBOY_COVER_FLAVOR=nonexistent`; confirm process exits at startup with the expected `ConfigError` message.
...
- [x] (2026-05-07) **Step 14.1:** Mark every task `[x]` in this plan with completion date.
- [x] (2026-05-07) **Step 14.2:** Move feature and plan to `done/` per CLAUDE.md naming convention.
- [x] (2026-05-07) **Step 14.3:** Update `STATUS.md`: remove PB-026 from Active Work, add to Completed table with date and plan-archive link.
- [x] (2026-05-07) **Step 14.4:** Append CHANGELOG entry: feature complete summary (visual change at the default 1264 × 1680 resolution; flavor system available for future extension; legacy `cover-templates.ts` removed).
- [x] (2026-05-07) **Step 14.5: Pre-PR checklist** (CLAUDE.md): all tasks closed, fixtures committed, sonar reviewed, ready for PR.
- [x] (2026-05-07) **Step 14.6:** Open PR (only after explicit user approval per CLAUDE.md rule 15).
...
- [x] (2026-05-07) Docker image growth ≤ 10 MB versus pre-feature baseline. Measure: `docker build` before and after; compare image sizes.
- [x] (2026-05-07) Render time at default resolution ≤ 200 ms per article. Measure: log timestamps in `MarkdownEpubConverter.toEpub` around the cover-generation block; convert one representative article; assert.
- [x] (2026-05-07) All bundled flavors render correctly at all three resolutions (Task 7's multi-resolution test).
- [x] (2026-05-07) No new `any`, `as`, or `@ts-ignore` introduced. Audit: `grep -nE "\bany\b|@ts-ignore|@ts-expect-error|\bas \b" src/` shows no new entries from this branch.
- [x] (2026-05-07) `npm audit` clean.
- [x] (2026-05-07) `flavors/classic/` imports nothing from Satori or sharp (lint rule from Task 10 enforces).
