...
- [x] Every EPUB produced by paperboy includes a cover derived from the document title and author
- [x] The cover is generated automatically — no new required parameters or configuration
- [x] The cover renders correctly on Kindle devices (tested via Send to Kindle email)
- [x] No regression in existing tests; new tests cover the cover generation path
